...
block_size = 16
key_size = 32
#reference from https://github.com/sommer/loxodo/blob/master/src/twofish/twofish.py
class Twofish:
...
def write():
    while True:
        ...
        message = Twofish(key).encrypt(message)
        message = username + ':' + str(message)
...
def receive():
    while True:
        try:
            ...
            if ":" in message:
                fr = message[:message.find(":")+1]
                message = message[message.find(":")+1:]
                
                message =ast.literal_eval(message)
                replain = str(Twofish(key).decrypt(message))
                replain = replain[2:-1]
                replain = unpadding(replain )
